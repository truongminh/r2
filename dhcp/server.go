...
func Start(config ServerConfig) {
	handler := &DHCPHandler{
		...
		options: dhcp.Options{
			dhcp.OptionSubnetMask:       config.SubnetMask,
			dhcp.OptionRouter:           config.RouterIP,    // Presuming Server is also your router
			dhcp.OptionDomainNameServer: []byte{8, 8, 8, 8}, // Presuming Server is also your DNS server
		},
	}
	log.Fatal(dhcp.ListenAndServeIf(config.Interface, handler))
}
...
func (h *DHCPHandler) ServeDHCP(p dhcp.Packet, msgType dhcp.MessageType, options dhcp.Options) (d dhcp.Packet) {
	// log.Printf("dhcp msgType=%d", msgType)
	switch msgType {
